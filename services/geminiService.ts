...
// Fix: Use the defined global variable for apiKey
...
export const getConsultationAdvice = async (userQuery: string): Promise<string> => {
  if (!__GEMINI_API_KEY__) return "I'm sorry, the consultation AI is currently offline (Missing API Key).";
...
export const translateBlogContent = async (
  ...
  if (!__GEMINI_API_KEY__) return null;
